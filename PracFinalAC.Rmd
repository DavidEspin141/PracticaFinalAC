...
credit.Datos.Train.TratamientoOUT_NA <- credit.Datos.Train
...
"tratamiento inicial" que el resto de modelos previamente mencionados.
...
```{r}
#borramos los datos missing del conjunto de entrada
credit.Datos.Train_Borrar_NA <- na.omit(credit.Datos.Train)
...
89% con mtry=8; en el segundo, el mejor resultado ronda el 88%; mientras
que en el último modelo explorado, se obtiene un índice del 86%.
...
tienen importancia negativa (lo que se supone que influye de forma
negativa en el resultado del modelo), sin embargo haciendo diversas
pruebas borrenado estos, no obtenemos mejores resultados.

Ahora vamos a hacer un tratamiento diferente, en el que en vez de
eliminar los datos missing, vamos a aplicar el mismo tratamiento
anteriormente usado en modelos anteriores, es decir imputar por la
mediana en el caso de los datos numéricos (aunque también podríamos por
la media debido a la gran robustez del modelo frente a outliers), y por
la moda en el caso de los datos categóricos.

```{r}
set.seed(1234)
modelo.rf.TratarDatos <- train(
  x = credit.Datos.Train.TratamientoOUT_NA[credit.varsEntrada],
  y = credit.Datos.Train.TratamientoOUT_NA[[credit.varSalida]],
  method = "rf",
  trControl = trainControl(method = "cv", number = 5),
  verbose = FALSE
)
...
entrenamiento en este caso ronda el 87%.
...
set.seed(1234)
pred_4r.SinTratarV2 <- predict(modelo.rf.SinTratarDatosV2, newdata = credit.Datos.Test[credit.varsEntrada])
...
Observamos que, con el modelo tratando los datos NAs con imputación,
obtenemos un índice de certeza del 85,4%, lo que resulta un índice
...
podremos modificarlos en un futuro.  Para empezar con el entrenamiento,
